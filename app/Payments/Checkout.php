...
class Checkout extends BaseController
{
    public function create_order($request, $response, $args)
    {
		...
		$Pending = TempOrder::where('userid', $user->id)->where("status", 0)->orwhere("status", -1)->count();
		if($Pending > 0){
        ...
        if ($code == '') {
			...
			if($usage > 0){
				$use_count=Order::where("userid",$user->id)->where("coupon",$coupon->code)->where("state",1)->count();
				if($use_count>=$usage){
					$res['ret'] = 0;
	...
	public function check_order($request, $response, $args)
    {
		...
		$order = TempOrder::where("userid",$user->id)->where("order_id", $orderid)->where("status", 0)->orwhere("status", -1)->first();
		if ($order == null || $order == ""){
			$newResponse = $response->withStatus(302)->withHeader('Location', '/portal/packages');
			return $newResponse;
		}
    ...
    public function setExpire($request, $response, $args)
	{
		...
		$order = TempOrder::where("order_id", $orderId)->where("status", 0)->first();
        if ($order){
